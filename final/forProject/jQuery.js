$(document).ready(function(){
	var state_toggle=1;
	var test = 1;
	var i = 8, j = 8, prei = 0, prej = 0, lasti = 0, lastj = 0;
	var value = i + '-' + j;
	...
	$("#suggest").click(function()
	{
		test = 0;
		var sug = computer('＋');
		sugi = sug[0], sugj = sug[1];
		var value = sugi + '-' + sugj;
		console.log(sugi, sugj);
		$('td[weizhi="'+value+'"]').html('*');
		$('td[weizhi="'+value+'"]').css({"background-color":"rgb(155, 255, 255, 0.5)"});
	})
	$("#hideSuggest").click(function()
	{
			...
			{			
				...
				for (var k = i - 2; k < i + 2; k++)
				{
					...
					for (var q = j - 2; q < j + 2; q++)
						...
						var temp = k + '-' + q;
						current += deadFore('td[weizhi="'+temp+'"]', target);
						
						//console.log(k,q);}
					}
				}
				$('td[weizhi="'+value+'"]').html(counter);
				for (var k = i - 2; k < i + 2; k++)
				{
					if (k < 1)
					{
